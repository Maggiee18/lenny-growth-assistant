...
- **The full Docker Compose stack (Postgres+pgvector, Ollama, backend, frontend) has been run live end-to-end**, including real ingestion (26 documents / 1,455 chunks) and real grounded Q&A/Ship 30/artifact generation against `llama3.1:8b` on CPU. Doing this surfaced and fixed 6 real bugs that the SQLite-only test suite could not have caught: a missing pgvector comparator on a custom column type, a naive/aware datetime mismatch, an unhealthy-but-working Ollama healthcheck, a missing `frontend/public/` directory, a too-short model timeout, and — the most serious one — an `ivfflat` vector index so badly over-tuned for this dataset size (100 list-clusters for ~1,455 rows) that it could return **zero** results for a real, well-supported question; fixed by dropping it in favor of exact search, which is both correct and fast enough at this scale. Full account in `agent-transcripts/009-live-docker-verification.md` and `010-ivfflat-retrieval-bug.md`. **Two things from that live run are worth knowing before you rely on this**: (1) on CPU-only inference, a plain grounded Q&A turn took ~54s and a Ship 30 essay took 3-6 minutes — noticeably slower than the original 15s target, budget for it; (2) a live Ship 30 essay's continuation pass introduced one ungrounded anecdote (an Airbnb example not present in any retrieved source) before the prompt was tightened to explicitly forbid that — the fix is deployed but was not re-verified with another full live generation, so treat generated essays as something to spot-check, not blindly trust, consistent with the grounding contract being a prompt-level mitigation rather than a hard guarantee (see PRD.md "Risks").
- Artifact generation (Markdown/HTML) used to silently proceed with no grounding material at all when retrieval found nothing and no conversation history was available — producing artifacts like one literally titled "No Transcript Excerpts Provided". Fixed: both tools now abstain outright when there's neither relevant transcript evidence nor prior conversation to draw on, and can ground in conversation history (not just transcripts) for requests like "summarize this conversation" — matching the assignment's explicit allowance for artifacts to be based on "the current conversation and/or grounded content".

...
| Metric | Target | Measured (live Docker + Ollama, 2026-09-13/14, CPU-only 8B model) |
|---|---|---|
| Grounded answer rate (non-abstained answers include >=1 cited source) | >= 90% for in-scope questions | 2/2 real in-scope questions tested live returned non-abstained answers with 3-6 real cited sources each (small sample -- a full `eval/run_eval.py` pass across all categories was not run this session; see below). |
| Unsupported-answer / hallucination rate | 0% fabricated quotes/stats | **Not 0% in practice.** A live Ship 30 essay's bounded continuation pass introduced an Airbnb/Chesky-Gebbia anecdote absent from every retrieved source for that question -- a real, observed grounding violation, not a hypothetical risk (see `agent-transcripts/009-live-docker-verification.md`). Fixed by explicitly forbidding new examples in the continuation prompt; the fix was deployed but not re-verified with another full live generation. The code-level guard (retrieval count < 2 -> refuse) worked correctly in every test; the gap is prompt-level fidelity within a generated answer, which is inherently probabilistic, not a hard guarantee. |
| Retrieval hit rate on in-scope questions | >= 80% | 2/2 tested live returned relevant sources (relevance scores 0.56-0.61 on the first, similarly reasonable on the follow-up). `eval/run_eval.py`'s full 9-category run was not executed this session -- recommended as the next step for a fuller measurement. |
| Task completion rate (user gets a usable artifact on request) | >= 95% | Ship 30: 3/3 live attempts produced a saved, viewable artifact (none failed outright); word-count-in-band success rate was 1/3 on the first try, reaching band only after two rounds of live iteration (prompt strengthening, then a bounded continuation pass) -- see finding 6 in `009-live-docker-verification.md`. Markdown/HTML artifact generation was verified via automated tests and the SQLite-backed live smoke test, not re-verified against real Ollama output this session. |
| Time from question to grounded answer | < 15s on a mid-range laptop with an 8B Ollama model | **Measured, and the target was wrong for CPU-only inference**: a plain grounded Q&A turn took ~54 seconds on CPU (retrieval: 73ms, generation: ~54s), and a full two-stage Ship 30 essay took 172-364 seconds depending on whether a continuation pass fired. Revised, honest expectation for this hardware profile: 30-90s for a Q&A turn, several minutes for a Ship 30 essay. A GPU-accelerated Ollama setup would be meaningfully faster; this was tested CPU-only. |

...
| 6 | Agent transcripts | [agent-transcripts/](../agent-transcripts/) (10 logs, failures included — including a full live Docker/Ollama debugging session in `009-live-docker-verification.md` and a serious retrieval-index bug in `010-ivfflat-retrieval-bug.md`) |
| 7 | Tests + manual UI test plan | [backend/tests/](../backend/tests/) (89 tests), [frontend/tests/](../frontend/tests/) (11 tests), [design.md § Manual UI test plan](../design.md#manual-ui-test-plan) |
...
**Executed and verified live, for real, on the real stack, culminating in a full green eval run:** `docker compose up --build` (all 4 services healthy), real Postgres+pgvector, real Ollama (`llama3.1:8b` + `nomic-embed-text`), real ingestion (26 documents / 1,455 chunks, zero errors), and a complete `eval/run_eval.py` 9-question pass scoring **12/12 assertion checks (100%)**, `retrieval_hit_rate: 1.0`, `artifact_generation_success_rate: 1.0` — grounded Q&A with real cited sources, correct abstention on unsupported/weakly-supported questions, a follow-up question in-session, a Ship 30 essay landing at 1,068 words (inside the 938-1,563 target band), and Markdown/HTML artifact generation all confirmed working together in one run. Getting to that clean run found and fixed 7 real bugs the SQLite test suite structurally couldn't catch — most notably a badly over-tuned `ivfflat` vector index that could return zero results for a valid query, and an abstention-detection gap where a model correctly declining to answer from an irrelevant retrieved chunk wasn't being recorded as "abstained". Full account across `agent-transcripts/009-live-docker-verification.md` and `010-ivfflat-retrieval-bug.md`. Also: all 91 backend tests + 6 eval-script tests (SQLite), frontend `tsc`/Vitest/`next build`.

**Not executed this session:** real Anthropic/cloud provider output (only Ollama was tested live, `ANTHROPIC_API_KEY` was left unset); the demo video recording. **One caveat that held up under re-testing but isn't proven solved**: a Ship 30 continuation pass was observed introducing one ungrounded anecdote early in this session; the prompt fix has now been exercised across several subsequent live Ship 30 generations (including the one behind the passing eval run) with no recurrence — a good sign, not a guarantee. See README § Known limitations and PRD.md § Risks for the full honest framing, including a note on real run-to-run variance observed on borderline/weakly-supported questions.

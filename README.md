...
- **The full Docker Compose stack has been run live end-to-end and verified with a full evaluation pass.** `docker compose up --build`, real ingestion (26 documents / 1,455 chunks), and real grounded Q&A/Ship 30/artifact generation against `llama3.1:8b` on CPU all work. `eval/run_eval.py`'s full 9-question run currently scores **12/12 assertion checks passing (100%)**, `retrieval_hit_rate: 1.0`, `artifact_generation_success_rate: 1.0` — see `eval/results.json` after running it yourself, and PRD.md "Success metrics" for the full breakdown. Getting there took fixing 7 real bugs the SQLite-only test suite could not have caught (full accounts in `agent-transcripts/009-live-docker-verification.md` and `010-ivfflat-retrieval-bug.md`): a missing pgvector comparator on a custom column type, a naive/aware datetime mismatch, an unhealthy-but-working Ollama healthcheck, a missing `frontend/public/` directory, a too-short model timeout, artifact generation proceeding with no grounding material at all (fixed: it now abstains, and can ground in conversation history for requests like "summarize this conversation"), and — the most serious one — an `ivfflat` vector index so badly over-tuned for this dataset size (100 list-clusters for ~1,455 rows) that it could return **zero** results for a real, well-supported question, fixed by dropping it in favor of exact search.
- **On CPU-only inference, budget real time**: a plain Q&A turn takes roughly 1-2 minutes, a Ship 30 essay ~6 minutes, artifact generation 40s-2min — materially slower than the original 15s target in early planning. A GPU-accelerated Ollama setup would be faster; only CPU was tested.
- One hallucination was observed and fixed during development (a Ship 30 continuation pass inventing an ungrounded Airbnb anecdote) and has not recurred across three subsequent live Ship 30 generations since — but the grounding contract remains a prompt-level mitigation, not a hard guarantee, so treat generated essays as something to spot-check rather than blindly trust (see PRD.md "Risks").
- Borderline/vague questions show real run-to-run variance: the same weakly-supported eval question sometimes gets a hedged answer with sources and sometimes correctly abstains, depending on LLM sampling. Not a bug — declining on weak evidence is the more conservative, arguably more correct behavior — but worth knowing this system isn't perfectly deterministic even against identical ingested data.
